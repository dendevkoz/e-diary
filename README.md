...
- в терминале запустите интерактивный режим Django shell
  ```
  python3 manage.py shell
  ```
  
- в терминале Django shell подключите скрипт
  ```
...
Файл `good_boy.py` содержит функции для исправления оценок, удаления замечаний, генерации похвалы.

  - Функция изменяет все оценки ниже 4 на 5 для заданного ученика.
    ```
  ...
  - Функция создает похвалы от учителя по определенному предмету для заданного ученика.
    Текст похвалы берется рандомным способом из списка в файле good_boy.py. Все похвалы взяты у отличников.
    ```
